...
void resultMenuCatch(int res, MyPriorityQueue<Document> & spooler, std::mutex &mu)
{
	...
	MyPriorityQueue<Document>::ConstIterator it = spooler.cibegin();
	int spoolerSize = spooler.getSize();
	gtoxy(40, 10, 40, 14, 7);
	for (int a(0); a < spoolerSize && a < 14; ++a, ++it) // a < 14 - щоб за межі екрана не вийшов!
	{
		gtoxy(40, 10 + a, 7);
		std::cout << "Add: \"" <<  it->->getName()  /* (*it).getName() */ << "\" size: " << (*it).getSize() << " by " << owners2[(*it).getOwner()];
	}
	mu.unlock();
}
...
void tr2_add_queue_menu(Menu &m, MyPriorityQueue<Document> & spooler, std::mutex &mu)
{
	...
	{
		menuRes = m.runMenu();
		resultMenuCatch(menuRes, spooler, mu);
		mu.lock();
		gtoxy(0, 0, 80, 10, 7);
		mu.unlock();
	} while (menuRes);
