...
public:
	...
	typedef typename std::pair<const typeKey*, typeVal*> pval;
	...
	const val operator[](size_t index) 
	{
		if (index >= size)
	...
	pval findByKey(const typeKey &k) 
	{
		pval tpval;
		...
		{
			tpval.first = nullptr;
			tpval.second = nullptr;
		}
		else
		{
			tpval.first = &res->nodeKey;
			tpval.second = &res->nodeVal;
		}
		return tpval;
	}
