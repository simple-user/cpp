...
public:
	...
	unsigned char height() // повертаЇ висоту всього дерева. дл€ nullptr висота - 0
	{
		return root ? root->height : 0;
	}
	unsigned char height(node* p) // повертаЇ висоту дл€ даного вузла. дл€ nullptr висота - 0
	{
		return p ? p->height : 0;
	}
...
private:
	...
	//node* insert(const typeKey &k, const typeVal &v) // вставка ключа k в дерево с корнем p
	//{
	//	node* p = root;
	...
	node* remove(const typeKey & k) // удаление ключа k из дерева p
	{
		node* p = root;
		if (!p) return 0;
		if (k < p->key)
