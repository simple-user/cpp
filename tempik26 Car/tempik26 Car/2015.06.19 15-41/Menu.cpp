...
bool Menu::addMenu(int addr)
{
	...
	int parentIndex = 0;
	for (int a(0); a < arrAddrLocal.size() - 1; a++)
...
void Menu::printMenushka(bool gray, int indMenu) const
{
	...
	for (int a(0); a < menushka.arrPunkt.size(); a++)
	{
		...
		for (int b = menushka.arrPunkt[a].strPunkt.size(); b < menushka.maxLenPunkt; b++) //добиваємо пробілами (для рівності виводу)
			cout << ' ';
		cout << endl;
	}
}
...
void Menu::printMenushkaBlack(int indMenu) const
{
	...
	for (int a(0); a < menushka.arrPunkt.size(); a++)
	{
		...
		for (int b = 0; b < menushka.maxLenPunkt; b++) //забиваємо пробілами (все)
			cout << ' ';
		cout << endl;
	}
}
...
void Menu::goUp()
{
	if (menu[activeMenushkaIndex].activePunktIndex>0)
	{
		...
		for (int b = menushka.arrPunkt[menushka.activePunktIndex].strPunkt.size(); b < menushka.maxLenPunkt; b++) //добиваємо пробілами (для рівності виводу)
			cout << ' ';
		cout << endl;
