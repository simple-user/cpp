...
Snake::Snake(size_t size, Pole &p, Nyamka &n)
: snakeDir(LEFT), indexHead(0), pole(&p), nyamLink(&n), dirUp(true), colorSnake(7), startSize(size)
{
...
void Snake::init()
{
	...
	dirUp = true;
	colorSnake = 7; 
	for (size_t a = 0; a<startSize; ++a)
	{
		COORD co = { pole->width / 2 + a, pole->height / 2 };
		snakeArr.push_back(co);
		showOne(co);
		pole->setCoord(co.X, co.Y, true, colorSnake, snakeZnak);
	}
}
...
int Snake::move()
{
	...
	Dir tempDir = snakeDir;

	int res = 0; // -1 - клацнув сам себе, 0 - все норм, 1 - з"їв нямку(ще не знаю де це використати)
	bool temp =0; // for help :)
	bool escape = false;
	do
	{
		while (!_kbhit() && res != -1 && !(res == 1 && snakeArr.size() == pole->height * pole->width))
		{
			if (clock() - waitStep >= snakeSpeed)  //треба зробити крок
			{
				...
				waitStep = clock();
				// auto mode :)
				//if ((snakeArr[indexHead].X == 0 || snakeArr[indexHead].X == pole->width - 1 ) && !temp)
				//{
				//	temp = 1;
				//	snakeDir = UP;
				//}
				//else if (snakeArr[indexHead].X == 0 && temp)
				//{
				//	temp = 0;
				//	snakeDir = RIGHT;
				//}
				//else if (snakeArr[indexHead].X == pole->width - 1 && temp)
				//{
				//	temp = 0;
				//	snakeDir = LEFT;
				//}
		...
		case 224:
			...
			{
				//tempDir = static_cast<Dir>(mask2.at(key[1]));
			...
			break;
		case 27: // ESC
			escape = 1; // про цьому res має бути 0
			res = 0;
			break;
		}
	} while (!escape);
