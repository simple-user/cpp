...
BinaryTreeInt::Node::Node( int key, const string & value )
	...
	, _key( key), _val( value )									// але містить ключ і значення
{}
...
ostream & operator << ( ostream & out, BinaryTreeInt::Node * node )
{
	...
	out <<"Current node : key=" <<node->_key <<" value=\"" <<node->_val <<"\"\n";

	if( node->_less != nullptr )
	{
		out <<"        less : key=" <<node->_less->_key <<" value=\"" <<node->_less->_val <<"\"\n";
	}
	else
	{
		out <<"        less : empty\n";
	...
	{
		out <<"        more : key=" <<node->_more->_key <<" value=\"" <<node->_more->_val <<"\"\n";
	}
	else
	{
		out <<"        more : empty\n";
