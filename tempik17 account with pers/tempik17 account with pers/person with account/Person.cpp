...
//Person();
//Person::Person() : Fam(nullptr), Imn(nullptr), Otc(nullptr), DrStr(nullptr), Sex(MALE), PhoneNumber(nullptr)
//{
//	Dr = {};
//}
...
// inline void copyDateFromPerson(const Person &p);
inline void Person::copyDateFromPerson(const Person &p)
{
	if (p.DrStr)
	{
		if (!DrStr)
			DrStr = new char[DATE_STR_LEN];
		strcpy_s(DrStr, 11, p.DrStr);
		Dr = p.Dr;
	}
	else if(DrStr)
	{
...
//inline void copyPhoneFromPerson(const Person &p);
inline void Person::copyPhoneFromPerson(const Person &p)
{
	if (PhoneNumber && p.PhoneNumber && strlen(PhoneNumber) != strlen(p.PhoneNumber))
		strcpy_s(PhoneNumber, strlen(PhoneNumber) + 1, p.PhoneNumber);
	else
		strInput(PhoneNumber, p.PhoneNumber);
...
Person::Person(const char *fam, const char *imn, const char *otc, const char* dr, const sex _sex, const char *phoneNumber,
	           const char* account_nom, char account_curr[], int account_moneySum)
			   : Person_account(account_nom, account_curr, account_moneySum)
{
	assign(fam, imn, otc, _sex, phoneNumber);
	makeDateFromStr(dr);
	makePhoneFromStr(phoneNumber);
...
//: Fam(nullptr), Imn(nullptr), Otc(nullptr), DrStr(nullptr), PhoneNumber(nullptr)
//{
//	assign(p.Fam, p.Imn, p.Otc, p.Sex, p.PhoneNumber);
//	copyDateFromPerson(p);
//	copyPhoneFromPerson(p);
//}
...
{
	movePerson(p);
	Person_account = move(p.Person_account);
	return *this;
}
