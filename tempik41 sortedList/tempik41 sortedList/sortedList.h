...
public:
	...
	// typedef typename sortedList<T>::indexator indexator;
	...
	size_t insert(const T &val)  // повертає номер позіції (індекса) в яку вставив
	{
		...
		// якщо непустий
		while (t && t->val<val+1)  // t && t->val<=val
		{
			t = t->next;
			++pos;
		}
	...
	//reference at(size_t index) тупняк. це теж треба засовувати в індексатор
	//{
	//	return getPtrByIndex(index)->val;
	//}
