...
public:
	...
	bool tratata(size_t count, std::vector<Res> &vecRes)
	{
		...
		{
			vecRes.emplace_back(pMag->top()->getNumber(), pMag->top()->pif());  // а ось і приклад коли корисно мати функцію top() 
			                                                                    // якби елемент контейнера вертався би по pop() ми могли б викликати тільки 1 раз
																				// і тому довелось би вводити тимчасову змінну
			pMag->pop();
		}
		return true;
	}
